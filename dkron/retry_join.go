...
func (r *retryJoiner) retryJoin() error {
	...
	log.Info("[INFO] agent: Retry join %s is supported for: %s", r.cluster, strings.Join(disco.Names(), " "))
	log.Info("[INFO] agent: Joining %s cluster...", r.cluster)
	attempt := 0
	for {
		var addrs []string
		var err error
		...
		for _, addr := range r.addrs {
			...
			case strings.Contains(addr, "provider="):
				...
				if err != nil {
					log.Error("[ERR] agent: Join %s: %s", r.cluster, err)
				} else {
					addrs = append(addrs, servers...)
					log.Info("[INFO] agent: Discovered %s servers: %s", r.cluster, strings.Join(servers, " "))
				}
		...
		if len(addrs) > 0 {
			n, err := r.join(addrs)
			if err == nil {
				log.Info("[INFO] agent: Join %s completed. Synced with %d initial agents", r.cluster, n)
				return nil
			}
		}
		...
		log.Warning("[WARN] agent: Join %s failed: %v, retrying in %v", r.cluster, err, r.interval)
